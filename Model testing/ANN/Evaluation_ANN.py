...
def preprocess_data(df, encoder=None):
    """Replicate preprocessing from training script - Modified for ANN"""
    ...
    # The 3D reshape specific to LSTM is removed.
    # Standard ANNs typically expect 2D input: (samples, features)
    # X_reshaped = X.reshape((X.shape, 1, X.shape)) # <-- This line is removed/commented out

    # Return the 2D normalized data X, suitable for standard ANN input
...
def main(model_path, test_file):
    ...
    # Load model
    # Ensure the model saved at model_path is an ANN/DNN model
    model = load_model(model_path)
    model_name = os.path.basename(model_path)

    # Load and preprocess test data using the ANN-compatible function
    test_df = load_test_data(test_file)
    X_test, y_test, _ = preprocess_data(test_df) # Uses the modified preprocess_data
...
if __name__ == "__main__":
    if len(sys.argv)!= 3:
        print("Usage: python test_ann_model.py <path_to_model.h5> <test_file.csv>") # Suggest renaming the script
        sys.exit(1)
